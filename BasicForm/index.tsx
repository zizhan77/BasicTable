 ...
 * @LastEditTime : 2023-02-17 14:17:48
 * @FilePath     : /b2b_web/src/components/Builder/BasicForm/index.tsx
 */
...
const BasicForm: React.FC<props> = ({ url, }) => {
    console.log(url)
    ...
    const getInitData = () => {
        ...
        setpageLoading(true)
        getCreateFormData(url).then(res => {
            setpageLoading(false)
            if (res.code == 200) {

                const { controls, labelCol, wrapperCol } = res.data
                controls.map((res:any) => {
                    if (res.componentName == 'image' && res.mode != "multiple"){

                        setAvatarUrl(res.value)
                        console.log('aaa', res.value)
                    }
                })
    ...
    const checkTree = (name: string, keys: any) => {
        form.setFieldValue(name, keys)
    }
    return (
                            ...
                            controls.map((control: any) => {
                                ...
                                if (control.componentName == "tree") {
                                    return (
                                        ...
                                        >
                                            <Tree
                                                defaultCheckedKeys={control.value}
                                                checkable
                                                onCheck={(e) => { checkTree(control.name, e) }}
                                                ...
                                                <Upload
                                                    ...
                                                    onChange={(info: any) => {
                                                        ...
                                                        fileList = fileList.map((file: any) => {
                                                            if (file.response.image_url) {
                                                                file.url = file.response.image_url;
